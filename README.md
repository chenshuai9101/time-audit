...
时间审计填的是中间这层：**主动从你的日常使用数据里挖出可以让 AI 替你做的工作模式**。它不替你回忆，它替你诊断。

## 多源采集 · 命令/意图层（v2.3 新增）

只看屏幕 OCR 对重度终端 / Agent 用户是有盲点的——你真正反复做的事（敲命令、反复委托 agent）发生在终端和 agent 会话里，OCR 只能抓到一堆噪声。

v2.3 把数据源做成**可插拔的适配器框架**，任意"日常使用中可优化的流程化来源"都能接：

| 源 | 抓什么 | 价值 |
|:--|:--|:--|
| `screenpipe` | 屏幕 OCR | GUI 操作（原有能力） |
| `shell` | `~/.zsh_history` / bash | 你反复手敲的命令 |
| `claude` | `~/.claude/projects/*.jsonl` | 你反复让 Claude Code 做的事（自动跳过 claude-mem 注入噪声） |
| `openclaw` | `~/.openclaw/cron` | 你已固化的定时自动化任务 |

关键差异：**命令 / 意图层不交给 LLM 猜，而是确定性统计真实重复**——

- **点**：单条命令重复 ≥ N 次 → 建议 alias / snippet
- **线**：连续命令序列重复 ≥ N 次 → 建议脚本 / skill

每条证据都是**真实命令原文 + 真实次数**，零幻觉。这是它相对"OCR + LLM 让模型编证据"路线的根本优势。报告里每条洞察都带 `modality`（`command` / `ocr`）标明来源；OCR 层洞察落盘前会做**证据校验**，引用了不存在 session 的幻觉条目直接丢弃。

```bash
time-audit --dryrun --days 30                 # 多源采集 + 命令挖掘（不需要 LLM）
time-audit --days 30 --sources shell,claude   # 只用命令/意图层
```
...
返回结构遵循 **[Automation Opportunity Schema (AOS)](docs/automation-opportunity-schema.md)** —— 一份工具中立、带版本号的"自动化机会"规范（机器可校验 schema 见 [`docs/automation-opportunity-schema.json`](docs/automation-opportunity-schema.json)，当前 v0.1.0 Draft）。任何工具都可产出或消费 AOS，时间审计只是它的参考实现。
...
│   ├── sources/                  # ⭐ 多源适配器框架（v2.3）
│   │   ├── base.py               # SourceAdapter 统一接口
│   │   ├── registry.py           # 注册 / 合并 / 排序 / 降级
│   │   ├── screenpipe.py         # 屏幕 OCR（包装原有读取）
│   │   ├── shell_history.py      # zsh / bash 历史
│   │   ├── claude_code.py        # Claude Code 转录（跳过注入噪声）
│   │   └── openclaw.py           # OpenClaw cron 自动化任务（best-effort）
│   └── core/
│       ├── db_reader.py          # Screenpipe SQLite / CSV / 模拟数据
│       ├── event_compressor.py   # ⭐ 事件流压缩（OCR 会话）
│       ├── command_miner.py      # ⭐ 确定性命令/意图挖掘（v2.3，零幻觉）
...
│       └── report_builder.py     # 输出渲染 + 证据校验 + modality 合并
...
- [x] **v2.3** 多源采集（shell / Claude Code / OpenClaw）+ 确定性命令挖掘（零幻觉）+ 证据校验
- [ ] **v2.4** 意图层语义归一化（把措辞不同的同类 agent 请求聚成一类）
